...
//    }    
  ...
  //Bank3.writePort(1, 0x00);
  //Bank3.writePort(1, 0xF0); //first 4 bits are led source - pins 8-12
  Bank3.writePort(1, 0x00); //last 4 bits are led sink - pins 13-16
}
...
void updateMatrix(int row) {
  byte1 = 0x00;
  for (col = 0; col < 4; col++){
    byte1 += ( led_matrix[row*4+col] << col); //columns are wired backwards R-L on my board, hence 3-col
  }
  ...
  //Bank3.writePort(1, 0x00); //hard-reset previous pins
...
//    Bank3.digitalWrite(button, LOW);
//    button++;
//    if (button == 4) {
//      button = 0;
//      Bank3.digitalWrite(col+4, HIGH);
//      col++; 
//      if (col == 4) {
//        col = 0;
//      }
//      Bank3.digitalWrite(col+4, LOW);
//    }
//    Bank3.digitalWrite(button, HIGH);
//    
//    
//    led_matrix[button] = !step_matrix[button];
//    Bank3.digitalWrite
